...
- 开书设定、故事引擎、题材定位、核心卖点、人设关系和第一卷节拍，优先使用 `$fanqie-story-engine`。
- 章节蓝图、场景卡、六章推进包、卡文修复、爽点节奏和章末追读钩子，优先使用 `$fanqie-chapter-blueprint`。
- 正文草稿、修订稿、去 AI 味、移动端可读性、对话改写和事实不变的表达修复，优先使用 `$fanqie-prose-polish`。
- 这些 skills 只提供创作流程和质量检查，不改变团队权限：新增内容仍需标注 `proposed`，不得写入 Story Bible、项目长期记忆或 llmwiki，定稿前仍需 `Novel-Continuity-Validator` 门禁。
...
2. 使用 `$fanqie-story-engine` 生成候选人物设定、关键剧情走势、人物关系、场景设定、核心卖点和伏笔候选。
3. 每个新增设定标注 `proposed`。
...
1. 基于上下文包和 `Project working directory`，使用 `$fanqie-chapter-blueprint` 生成章节蓝图、场景卡、情绪曲线和结尾钩子。
2. 使用 `$fanqie-prose-polish` 生成正文草稿和创作说明，确保移动端读者能快速理解动作、责任、后果和爽点。
3. 根据验证报告使用 `$fanqie-prose-polish` 修订，不改变已确认事实；优先保留读者可懂、节奏紧和冲突后果清晰。
