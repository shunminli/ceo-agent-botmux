...
- `launch` 在 macOS `.app` bundle 可定位时使用 `open -na <App>.app --args --remote-debugging-port=<port>` 启动，并等待 `http://127.0.0.1:<port>/json/version` 可用后才返回成功；显式传入非 `.app` 的自定义 executable 时后台启动该 executable，不等待 GUI 进程退出。
...
6. `launch` 生成或执行豆包桌面端 CDP 启动命令，供 `cdp-app` 和 OpenCLI `doubao-app` 使用；`--relaunch` 会显式退出已有 Doubao 再重启，非 dry-run 会用 CDP `/json/version` 作为启动成功判定。
...
- `tests/test_doubao_cli.py`：真实 Python 模块入口、fake runner、缺依赖、launch dry-run 和自定义 executable 后台启动验证。
