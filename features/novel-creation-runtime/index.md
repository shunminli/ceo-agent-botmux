...
- `workflow-foundation-command` JSON：真实 `novel-story-foundation` workflow 的可审阅启动命令，包含 `params`、`command` 和 shell-safe `command_text`。
...
- `schemas/next-chapter-command.schema.json`：章节后 handoff 必填字段和基础类型契约，覆盖下一章目标、真实 BotMux workflow 命令、知识回写命令、prior context 和 source refs；本地 schema validator 会校验数组 `items`、`enum` 和数值范围等已声明约束。
