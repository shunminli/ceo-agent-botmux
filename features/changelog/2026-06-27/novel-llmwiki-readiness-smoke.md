...
- Runs write-after lint, using local `wiki-lint` fallback when the configured llmwiki executable has no `lint` subcommand, and runs `llmwiki reindex` through the configured executable.
