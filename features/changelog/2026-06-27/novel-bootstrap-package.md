...
This turns the next real project step into a single auditable entry point. It still does not approve long-lived llmwiki writes or run reindex; the approval package contains the exact `llmwiki-sync --approve --reindex` command to run after human review.
