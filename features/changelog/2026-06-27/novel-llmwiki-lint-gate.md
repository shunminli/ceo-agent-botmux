...
- Approved sync runs `llmwiki lint <workspace>` when `--lint` is supplied. If the installed llmwiki CLI does not expose a `lint` subcommand, it runs local `python3 -m botmux_novel wiki-lint --workspace <workspace>` as a fallback; if lint exits non-zero, the sync result is `failed`.
