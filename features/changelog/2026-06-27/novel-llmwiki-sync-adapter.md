...
- Can optionally run `llmwiki reindex <workspace>` with `--reindex` and write-after lint with `--lint`; lint falls back to local `wiki-lint` when the llmwiki CLI has no `lint` subcommand.
