...
| 本地开发环境 | PATH 中已有 Codex CLI 时接入成本最低。 |
...
```bash
$HOME/.botmux/bin/botmux workflow validate \
  workflows/novel-story-foundation.workflow.json
```
...
`python3 -m botmux_novel project-init` 用于创建单本小说的独立工作区、番茄发布目录、bot handoff 目录、llmwiki workspace 占位目录和默认 `.gitignore`。工具代码继续放在本仓库；正文项目建议放在仓库外的独立目录，需要版本控制时在小说目录单独建私有 git。建议提交 `bible/`、`manuscript/final/`、`publish/fanqie/`、`tracking/` 和 `comms/decisions/`；`runs/`、bot 原始日志、临时索引和 `wiki/llmwiki-workspace/` 默认本地管理。
...
6. 修改 workflow 后跑 `python3 -m botmux_novel readiness`，静态检查 `${params.*}` 和 `${node.output.*}` 是否指向已声明参数、上游依赖和 `outputSchema.properties` 字段，并用合成输出渲染每个节点 prompt / humanGate prompt。
...
- 可将 `lucasastorian/llmwiki` 安装到 `$HOME/.local/opt/llmwiki`，PATH 入口放在 `$HOME/.local/bin/llmwiki`。
...
- 已生成 `workflows/novel-story-foundation.workflow.json`，可同步安装到 `$HOME/.botmux/workflows/novel-story-foundation.workflow.json`。
...
- 已新增本地 `python3 -m botmux_novel approval-check`，默认只读校验 `novel-bootstrap` 审批包；`--apply-dry-run` 可验证 approved apply 消费路径仍停留在 planned 模式，`--chapter-smoke` 仅用于临时 smoke 项目执行首章命令。
- 已新增本地 `python3 -m botmux_novel approval-apply`，读取 `novel-bootstrap` 审批包，默认 dry-run，传 `--approve` 后按包内 project/slug/workspace/llmwiki 配置执行 gated sync，并默认运行 reindex/lint。
- 已补充 `docs/novel-llmwiki-setup.md`，说明本地 workspace、MCP 权限、humanGate 和 lint 接入流程。

### Phase 2：章节生产 workflow

- 已生成 `workflows/novel-chapter-production.workflow.json`，可同步安装到 `$HOME/.botmux/workflows/novel-chapter-production.workflow.json`。
...
- `chapter` 会自动读取早于当前章节的 `runs/archive-*.json`，生成 `runs/{chapter_run_id}/prior-context.json` 并注入上下文包，避免第二章以后丢失前文事实、伏笔和角色状态；章节完成后的 `next-chapter-command` 同时包含本地 runtime 命令、真实 `novel-chapter-production` BotMux workflow 命令和章节后知识回写命令，便于从本地 smoke 切回三 bot 协作并生成 wiki 审核包/dry-run sync 计划。
- 已新增本地 `python3 -m botmux_novel chapter-workflow-import`，把真实 `novel-chapter-production` workflow 输出导入为本地 draft/revised/final、tracking 归档、SQLite run 记录和下一章 handoff；只有 Director 决策和 `archive_plan` 均通过时才写 final，被 block 的章节只写 blocked run artifacts。成功导入后会从 `runs/archive-*.json` 汇总 `project.yaml.archived_chapters`，并在下一章 BotMux 命令中携带本章归档摘要作为 `priorContext`，同时带上下一章目标、`wordTarget` 和 `mode`；同一 handoff 还会给出章节后 wiki bundle、dry-run llmwiki sync 和 humanGate 后 approved sync 命令。
...
| 输出格式不一致 | 统一 `novel_agent_output_v1`，workflow 中用 outputSchema 约束，并由 readiness 静态检查模板绑定和合成契约 smoke。 |
