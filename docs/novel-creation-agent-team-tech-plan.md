...
- Harness 统一采用 **Codex CLI**，不维护多套 Harness 方案。
...
所有小说 bot 统一使用 **Codex CLI Harness**。
...
| `Novel-Creative-Architect` | Codex CLI | 只读 | 人物、剧情、关系、场景、章纲、正文草稿、修订建议。 |
...
`Novel-Creative-Architect` 只产出候选设定和草稿。它不能把新想法标成已确认事实，不能直接写项目记忆或 llmwiki。
...
| `Blueprint` | `Novel-Creative-Architect` | 章节蓝图、场景卡、情绪曲线、结尾钩子。 |
| `Draft` | `Novel-Creative-Architect` | 正文草稿和创作说明。 |
| `Review` | `Novel-Continuity-Validator` | 硬约束、事实、人物、时间线、设定检查。 |
| `Revise` | `Novel-Creative-Architect` | 修订稿、diff、修改理由。 |
...
```text
你是中文小说创意架构师。你负责人物、剧情、关系、场景、章纲、草稿和修订建议。你可以发散，但不能把建议伪装成已确认事实。所有新增设定必须标注为 proposed，所有覆盖旧设定必须列出影响面。你不写入项目记忆或 llmwiki。输出必须符合 novel_agent_output_v1。
```
...
6. 当前阶段先不写 workflow，等 3 个小说 bot 创建并拿到 `larkAppId` 后再生成。
...
- 创建 3 个小说 bot：`Novel-Director-Curator`、`Novel-Creative-Architect`、`Novel-Continuity-Validator`。
...
| 风险 | 控制 |
| --- | --- |
| 3 个 bot 仍然职责过宽 | 先跑 5 章样例，用实际瓶颈决定是否拆分。 |
...
1. 创建 3 个小说 bot，并记录各自 `larkAppId`。
2. 给 `Novel-Director-Curator` 配置 llmwiki MCP。
3. 给 `Novel-Continuity-Validator` 配置 llmwiki 只读能力。
4. 把第 12 节身份配置写入 bot role prompt。
5. 确认后再生成 `novel-story-foundation.workflow.json`，并运行 BotMux validate。
